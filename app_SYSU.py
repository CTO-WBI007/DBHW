...
def admin_dashboard():
    return render_template('Admin???.html')  # 显示管理员管理页面
...
def student_courses(student_id):
    ...
    cursor.execute("""
        SELECT c.course_id, c.course_name, c.credit, t.name AS teacher_name
        FROM course c
    ...
    return jsonify(courses) # SQL语句需修改
'''
...
'''
# 查询课程(who?)
@app.route('/admin_course/<string:course_name>', methods=['GET'])
def search_courses(course_name):
    cursor = conn.cursor(dictionary=True)

    cursor.execute("""
        SELECT course_id, course_name, credit, name AS teacher_name
        FROM course
...
# 选课操作(who?)
@app.route('/Admin_course', methods=['POST'])
def enroll_course():
    ...
    if cursor.fetchone():
        return jsonify({"error": "已选此课程"}), 400
    ...
    return jsonify({"message": "选课成功"}) # SQL语句需修改
'''
# 后端API (连接js)
